...
public class Solution1011 {
    ...
    public int shipWithinDays(int[] weights, int D) {
        int minShipNum = getMinShipNum(weights);
        while (true) {
            if (canShip(weights, D, minShipNum)) {
                return minShipNum;
            } else {
                minShipNum++;
            }
    ...
    private boolean canShip(int[] weights, int D, int skipNum) {
        int index = 0;
        int temp = skipNum;
        for (int i = 0; i < D; i++) {
            while (skipNum - weights[index] >= 0) {
                skipNum = skipNum - weights[index];
                index++;
    ...
    private int getMinShipNum(int[] weights) {
